...
export const GAME_THEMES: CurationTheme[] = [
  ...
  editorialNotes: {
    'DARK SOULS III': 'Miyazaki\'s finale distills the series into pure rhythm — every bonfire a punctuation mark. It opens the lineage because it\'s the most legible entry point into a famously hostile grammar.',
    'Elden Ring': 'Miyazaki with open-world scope; the hostility now has a horizon. The accessible face because exploration dilutes the punishment, yet the design DNA is fully intact.',
    'Bloodborne': 'FromSoftware drops the shield for aggression; Yharnam\'s gothic horror rewards the rush, not the retreat. It widens the mood from medieval to visceral — the most adrenal entry.',
    'Demon Souls': 'The 2009 original that started it: world-tending, fragile hubs, the template every later game quotes. The pivot because without it the rest of this list does not exist.',
    'Sekiro': 'Deflect over dodge; a samurai\'s posture duel replaces stat-building. The bridge pick — same cruelty, different grammar, proving the design could leave RPG conventions behind.',
    'Lies of P': 'Neowiz\'s Pinocchio reskin keeps the parry-first feel while streamlining the lore. It argues the lineage outlived its studio and became a genre others can author.',
    'Nioh': 'Team Ninja fuses Souls structure with loot-and-stamina action; busier, faster, more diabolical. The bridge because it shows the grammar bending toward character-action.',
    'Hollow Knight': 'Team Cherry\'s hand-drawn metroidvania inherits the hostility and the mystery, not the combat exactness. It closes the list as proof the feeling outgrew its engine.',
  }},
  {
