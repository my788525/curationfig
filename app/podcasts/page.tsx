...
import { PODCASTS_THEMES, MOOD_THEMES } from '@/lib/media/curation';
...
function cards(items: { title: string; creator?: string; cover?: string | null; url: string }[], n = 14) {
  return items
    .slice(0, n)
    .map((i) => ({ title: i.title, subtitle: i.creator || '', cover: i.cover, href: i.url || '/podcasts/' }));
}

export const metadata: Metadata = {
  title: 'Podcasts Curation',
  description:
  ...
  return (
    <>
      <section className="hero has-bg" style={{ ['--hero-img' as string]: "url('/images/hero/hero-film.webp')" } as React.CSSProperties}>
        <div className="wrap hero-inner">
          <div className="crumbs" style={{ color: '#ece6f5', margin: '0 0 10px' }}>
            <Link href="/" style={{ color: '#ece6f5' }}>Home</Link> / Podcasts
          </div>
          <h1 className="grad-text">Podcasts, by theme</h1>
          <p>
            {PODCASTS_THEMES.length} editorial themes about why podcasts belong together — by movement,
      ...
      <section className="marquee-band">
        ...
        <Marquee lanes={[{ label: 'Podcasts', href: '/podcasts/', cards: cards(PODCASTS_ITEMS) }]} />
      </section>
        ...
        <div className="wrap">
          ...
          <Reveal className="grid grid-2">
            {PODCASTS_THEMES.map((t) => (
              <Link key={t.slug} href={`/podcasts/${t.slug}/`} className="card theme-card">
                <h3>{t.title}</h3>
