...
export function generateStaticParams() {
  return PODCASTS_THEMES.map((t) => ({ slug: t.slug }));
}

export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  const t = PODCASTS_THEMES.find((x) => x.slug === params.slug);
  if (!t) return { title: 'Podcasts Curation' };
  return {
    title: t.title,
    description: t.intro,
    alternates: { canonical: `/podcasts/${t.slug}/` },
  };
}
...
export default function BooksThemePage({ params }: { params: { slug: string } }) {
  ...
  const itemsBySeed = new Map(PODCASTS_ITEMS.map((i) => [(i.seedName || i.title).toLowerCase(), i]));
  ...
  const collLd = {
    ...
    name: t.title,
    description: t.intro,
    url: `https://curationfig.com/podcasts/${t.slug}/`,
  };
    ...
    <section className="section">
      <div className="wrap">
        ...
        <div className="crumbs">
          <Link href="/">Home</Link> / <Link href="/podcasts/">Podcasts</Link> / {t.title}
        </div>
        <h1>{t.title}</h1>
        ...
        <div style={{ marginTop: 22, display: 'flex', gap: 14, flexWrap: 'wrap' }}>
          ...
          <Link href="/podcasts/" className="pill-tag">← All podcast themes</Link>
        </div>
