...
export function generateStaticParams() {
  return COMICS_THEMES.map((t) => ({ slug: t.slug }));
}

export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  const t = COMICS_THEMES.find((x) => x.slug === params.slug);
  if (!t) return { title: 'Comics Curation' };
  return {
    title: t.title,
    description: t.intro,
    alternates: { canonical: `/comics/${t.slug}/` },
  };
}
...
export default function BooksThemePage({ params }: { params: { slug: string } }) {
  ...
  const itemsBySeed = new Map(COMICS_ITEMS.map((i) => [(i.seedName || i.title).toLowerCase(), i]));
  ...
  const collLd = {
    ...
    name: t.title,
    description: t.intro,
    url: `https://curationfig.com/comics/${t.slug}/`,
  };
    ...
    <section className="section">
      <div className="wrap">
        ...
        <div className="crumbs">
          <Link href="/">Home</Link> / <Link href="/comics/">Comics</Link> / {t.title}
        </div>
        <h1>{t.title}</h1>
        ...
        <div style={{ marginTop: 22, display: 'flex', gap: 14, flexWrap: 'wrap' }}>
          ...
          <Link href="/comics/" className="pill-tag">← All comics themes</Link>
        </div>
