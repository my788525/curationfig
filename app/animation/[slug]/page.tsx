...
export function generateStaticParams() {
  return ANIMATION_THEMES.map((t) => ({ slug: t.slug }));
}

export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  const t = ANIMATION_THEMES.find((x) => x.slug === params.slug);
  if (!t) return { title: 'Animation Curation' };
  return {
    title: t.title,
    description: t.intro,
    alternates: { canonical: `/animation/${t.slug}/` },
  };
}

export default function BooksThemePage({ params }: { params: { slug: string } }) {
  const t = ANIMATION_THEMES.find((x) => x.slug === params.slug);
  if (!t) notFound();

  const itemsBySeed = new Map(ANIMATION_ITEMS.map((i) => [(i.seedName || i.title).toLowerCase(), i]));
  ...
  const collLd = {
    ...
    name: t.title,
    description: t.intro,
    url: `https://curationfig.com/animation/${t.slug}/`,
  };
    ...
    <section className="section">
      <div className="wrap">
        ...
        <div className="crumbs">
          <Link href="/">Home</Link> / <Link href="/animation/">Animation</Link> / {t.title}
        </div>
        <h1>{t.title}</h1>
          ...
          <div style={{ marginTop: 28 }}>
            ...
            {missing.length > 0 && (
              <p className="muted" style={{ marginTop: 10 }}>
                {missing.length} entries pending metadata resolution.
              </p>
            )}
          </div>
        )}
        ...
        <div style={{ marginTop: 22, display: 'flex', gap: 14, flexWrap: 'wrap' }}>
          ...
          <Link href="/animation/" className="pill-tag">← All animation themes</Link>
        </div>
